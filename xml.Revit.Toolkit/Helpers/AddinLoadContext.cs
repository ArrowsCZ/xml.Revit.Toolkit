...
public sealed class AddinLoadContext : AssemblyLoadContext
{
    ...
    private AddinLoadContext(Type type, string addinName)
        : base(addinName)
    {
    ...
    public static AddinLoadContext GetDependenciesProvider(Type type)
    {
        ...
        if (DependenciesProviders.TryGetValue(addinRoot, out var provider))
            return provider;
    ...
    /// </summary>
    public static ExternalDBApplicationResult Invoke(
        object instance,
        string methodName,
    ...
    )
    {
        ...
        Type[] methodParameterTypes = [typeof(UIApplication), typeof(CategorySet)];

        object[] methodParameters = [applicationData, selectedCategories];
    ...
    )
    {
        ...
        object[] methodParameters = [commandData, message, elements];

        var method = instanceType.GetMethod(
            nameof(IExternalCommand.Execute),
            MethodSearchFlags,
            null,
            methodParameterTypes,
            null
