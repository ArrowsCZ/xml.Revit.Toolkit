...
/// 功能类
/// </summary>
public sealed class XmlPushButtonData
{
    /// <summary>
    /// 路径
    /// </summary>
    ...
    public XmlPushButtonData(Type type)
    {
        ...
        if (Type != null)
        {
            ...
            var xmlAttribute = Type.GetCustomAttributesData()
                .FirstOrDefault(o => o.AttributeType.ToString().Equals(typeof(XmlAttribute).FullName));
            ...
            {
                ButtonName = xmlAttribute.ConstructorArguments.ElementAtOrDefault(0).Value?.ToString() ?? Type.Name;
                Tooltip = xmlAttribute.ConstructorArguments.ElementAtOrDefault(1).Value?.ToString() ?? Type.Name;
                Description = xmlAttribute.ConstructorArguments.ElementAtOrDefault(2).Value?.ToString() ?? "";
            }
#else
            ...
            var bitmap = GetLocalResourcesByImageName(name);
            Image = bitmap;
    ...
    {
        var uri = new Uri("pack://application:,,,/xml.Revit.Toolkit;component/Resources/icon.png", UriKind.Absolute);
    ...
    /// <param name="fileName"></param>
    /// <returns></returns>
    public static BitmapSource GetLocalResourcesByImageName(string fileName)
    {
        try
        {
    ...
    public PushButtonData CreatePushButtonData()
    {
        ...
        if (xmlHelpUrlAttribute != null)
            buttonData.SetContextualHelp(new ContextualHelp(ContextualHelpType.Url, xmlHelpUrlAttribute.HelpUrl));
